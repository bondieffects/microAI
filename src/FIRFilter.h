...
class FIRFilter {
  public:
    FIRFilter(int16_t *filter, uint8_t n_taps);
    int16_t process(int16_t input);

  private:
    int16_t *filter;    // Pointer to filter coefficients
    int16_t *buffer;    // Buffer to store previous inputs (delay line)
    uint8_t n_taps;     // Number of filter coefficients (taps)
};
